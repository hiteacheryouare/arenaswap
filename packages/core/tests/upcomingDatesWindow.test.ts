...
// What each zone below proves is still its two edges, and the days between them are contiguous by
// construction — so the edges are what these read, and contiguity is asserted separately. Every
// literal is untouched from when this asked for a range.
const edges = (dayKeys: string[]): string => `${dayKeys[0]}-${dayKeys[dayKeys.length - 1]}`;

const windowIn = (timeZone: string, iso: string, days: number): string[] => {
	setTimeZone(timeZone);
	return loadApiClient().buildUpcomingDayKeys(days, new Date(iso));
};

const rangeIn = (timeZone: string, iso: string, days: number): string => edges(windowIn(timeZone, iso, days));
...
describe('the days between the two edges', () => {
	...
	test('a month end is counted rather than subtracted from', () => {
		expect(windowIn('America/New_York', '2026-09-29T14:00:00.000Z', 3)).toEqual([
			'20260929', '20260930', '20261001', '20261002',
		]);
	});

	test('a leap day is a day', () => {
		expect(windowIn('America/New_York', '2028-02-28T14:00:00.000Z', 2)).toEqual([
			'20280228', '20280229', '20280301',
		]);
	});

	test('a single day window is one key, not two', () => {
		expect(windowIn('America/New_York', '2026-09-05T18:00:00.000Z', 0)).toEqual(['20260905']);
	});
});
...
describe('the scoreboard request in the viewer\'s own time zone', () => {
	test('a Tokyo viewer is offered the Eastern slate their own day opened on', async () => {
		...
		// One request per Eastern day of the window, each naming a single date, plus the undated
		// board the live games come from.
		const asked = fetchMock.mock.calls
			.map(([url]) => new URL(String(url)).searchParams.get('dates'))
			.filter((dates): dates is string => dates !== null);
		expect(new Set(asked).size).toBe(asked.length);
		expect(edges(asked.toSorted())).toBe('20260905-20260913');
	});
});
