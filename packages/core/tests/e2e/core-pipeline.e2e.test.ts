...
describe('core API + excitement e2e flow', () => {
	...
	it('keeps working leagues and session-disables PWHL after 4xx scoreboard failure', async () => {
		const nbaPayload = {
			leagues: [{ logos: [{ href: 'https://example.com/nba-logo.png' }] }],
			events: [
				basketballEvent({
					id: 'nba-live-fallback',
					state: 'in',
					period: 3,
					displayClock: '5:00',
					homeScore: 88,
		...
		const fetchSpy = jest.spyOn(global, 'fetch').mockImplementation(async (input: RequestInfo | URL) => {
			const url = toUrl(input);
			if (url.includes('/hockey/pwhl/scoreboard')) return jsonResponse({ message: 'not found' }, 404);
			if (url.includes('/basketball/nba/scoreboard')) return jsonResponse(nbaPayload);
			throw new Error(`Unexpected URL requested: ${url}`);
		});
		...
		expect(firstResult.leagueLogos).toEqual({ nba: 'https://example.com/nba-logo.png' });
		expect(fetchSpy.mock.calls.some(([input]) => toUrl(input as RequestInfo | URL).includes('/hockey/pwhl/scoreboard'))).toBe(true);

		fetchSpy.mockClear();
		fetchSpy.mockImplementation(async (input: RequestInfo | URL) => {
			const url = toUrl(input);
			if (url.includes('/hockey/pwhl/scoreboard')) {
				throw new Error('PWHL should have been session-disabled after the first 404');
			}
			if (url.includes('/basketball/nba/scoreboard')) return jsonResponse(nbaPayload);
			throw new Error(`Unexpected URL requested: ${url}`);
		});

		const secondResult = await fetchGamesWithLeagueLogos(['pwhl', 'nba'], { includeUpcoming: false });
		expect(secondResult.games.map(game => game.league)).toEqual(['nba']);

		const secondCallUrls = fetchSpy.mock.calls.map(([input]) => toUrl(input as RequestInfo | URL));
		expect(secondCallUrls).toHaveLength(1);
		expect(secondCallUrls[0]).toContain('/basketball/nba/scoreboard');
	});
});
