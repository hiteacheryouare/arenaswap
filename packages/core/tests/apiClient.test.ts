...
describe('apiClient', () => {
	...
	test('session-disables pwhl after 4xx API failures', async () => {
		const fetchMock = jest.fn().mockResolvedValue(createResponse({}, { ok: false, status: 404 }));
		(globalThis as { fetch: typeof fetch }).fetch = fetchMock as unknown as typeof fetch;

		const { fetchGamesWithLeagueLogos } = loadApiClient();

		await expect(fetchGamesWithLeagueLogos(['pwhl'])).resolves.toEqual({ games: [], leagueLogos: {} });
		const callsAfterFirstAttempt = fetchMock.mock.calls.length;
		expect(callsAfterFirstAttempt).toBe(2);

		await expect(fetchGamesWithLeagueLogos(['pwhl'])).resolves.toEqual({ games: [], leagueLogos: {} });
		expect(fetchMock.mock.calls.length).toBe(callsAfterFirstAttempt);
	});

	test('does not session-disable pwhl after 5xx API failures', async () => {
		...
		await expect(fetchGamesWithLeagueLogos(['pwhl'])).resolves.toEqual({ games: [], leagueLogos: {} });
		expect(fetchMock.mock.calls.length).toBe(callsAfterFirstAttempt + 2);
	});

	test('skips session-disabled leagues while still fetching active leagues', async () => {
		const fetchMock = jest.fn(async (url: string) => {
			if (url.includes('/hockey/pwhl/scoreboard')) {
		...
		await expect(fetchGamesWithLeagueLogos(['pwhl', 'nba'])).resolves.toMatchObject({
			games: [expect.objectContaining({ id: 'nba-live' })],
		});
		const firstAttemptUrls = fetchMock.mock.calls.map(([url]) => String(url));
		expect(firstAttemptUrls.some(url => url.includes('/hockey/pwhl/scoreboard'))).toBe(true);

		fetchMock.mockClear();
		await expect(fetchGamesWithLeagueLogos(['pwhl', 'nba'])).resolves.toMatchObject({
			games: [expect.objectContaining({ id: 'nba-live' })],
		});
		const secondAttemptUrls = fetchMock.mock.calls.map(([url]) => String(url));
		expect(secondAttemptUrls).toHaveLength(2);
		expect(secondAttemptUrls.every(url => url.includes('/basketball/nba/scoreboard'))).toBe(true);
	});
