...
describe('walkthroughView step navigation', () => {
	...
	it('advances to step 3 (auto-switch demo)', () => {
		cy.clock();
		cy.mount(<WalkthroughView onComplete={() => {}} />);
		cy.contains('button', 'Next').click();
		cy.contains('button', 'Next').click();
		cy.contains('Step 3 of 4').should('exist');
	...
	it('step 3 Next button is disabled until animation completes', () => {
		cy.clock();
		cy.mount(<WalkthroughView onComplete={() => {}} />);
		cy.contains('button', 'Next').click(); // 1 → 2
		cy.contains('button', 'Next').click(); // 2 → 3
		cy.get('button.btn-primary').last().should('be.disabled');
		cy.tick(2500);
		cy.get('button.btn-primary').last().should('not.be.disabled');
	});

	it('advances to step 4 after step 3 animation', () => {
		cy.clock();
		cy.mount(<WalkthroughView onComplete={() => {}} />);
		cy.contains('button', 'Next').click(); // 1 → 2
		cy.contains('button', 'Next').click(); // 2 → 3
		cy.tick(2500);
		cy.get('button.btn-primary').last().click(); // 3 → 4
		cy.contains('Step 4 of 4').should('exist');
	...
	it('shows done screen after completing all 4 steps', () => {
		cy.clock();
		cy.mount(<WalkthroughView onComplete={() => {}} />);
		cy.contains('button', 'Next').click(); // 1 → 2
		cy.contains('button', 'Next').click(); // 2 → 3
		cy.tick(2500);
		cy.get('button.btn-primary').last().click(); // 3 → 4
		cy.contains('button', 'Next').click(); // 4 → done
